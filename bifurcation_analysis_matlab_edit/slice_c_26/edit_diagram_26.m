...
back = load('slice_c_26_94_backward.mat');
forw = load('slice_c_26_94_forward.mat');

x_back = back.x;  s_back = back.s;  f_back = back.f;
x_for  = forw.x;  s_for  = forw.s;  f_for  = forw.f;

%% LOAD LIMIT CYCLE (from Hopf bifurcation)

lc = load('slice_c_26_94_cycle_from_hopf.mat');
...
% exportgraphics(gcf,'rez_c_26_2D.pdf','ContentType','vector')
...
function plot_branch_by_eigs(x_branch, f_branch, ax, lw, which)
    ...
    % compute the maximum real part of eigenvalues at each point
    ...
    % split the branch into segments based on stability changes
...
function plot_lc_envelope(lc, ax, lw)
    ...
    % ===== PARAMETER ROW =====
    qRow = 804;   % adjust if parameter index differs
    q_lc = x_lc(qRow,:);
    ...
    % compute distance from the trivial multiplier (1)
    d1 = abs(mu1 - 1);
    d2 = abs(mu2 - 1);
    ...
    % determine stability of the limit cycle
    isStableLC = abs(mu) < 1;
    ...
    % plot unstable segments first, then stable ones
    for pass = 1:2
        for b = 1:numel(breaks)-1
            ...
            % plot lower and upper envelope of the limit cycle
            plot(ax, q_lc(idx), v_min(idx), col, 'LineWidth', lw)
            plot(ax, q_lc(idx), v_max(idx), col, 'LineWidth', lw)
        end
    end

    % plot bifurcation points of the limit cycle (upper branch)
    for k = 1:length(s_lc)
...
function plot_bif_points_eq(x_branch, s_branch)
    ...
    for k = 1:length(s_branch)
        ...
        % extract bifurcation point coordinates
        q_bif = x_branch(3,idx);
        v_bif = x_branch(1,idx);

        % plot and label bifurcation point
